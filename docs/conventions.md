...
| 액션 | 목적지 | 방식 |
|---|---|---|
| HomeAIBanner 탭 | `/chat` `{ mode: 'new' }` | navigate |
...
- `app/(main)/chat/index.tsx` (`/chat`) — 새 채팅 생성 및 최근 채팅방 분기처리
- `app/(main)/chat/[chatId].tsx` (`/chat/[chatId]`) — 특정 채팅방 직접 진입

진입 방식에 따라 `mode` param 유무로 분기한다.

```tsx
// BottomNavigation 탭 — 분기 없이 /chat으로 이동
router.navigate('/chat');

// HomeAIBanner / NewTravelGenerateButton — 새 채팅 강제 진입
router.navigate({ pathname: '/chat', params: { mode: 'new' } });

// RecentChatSection 카드 탭 — 특정 채팅방 직접 진입
router.navigate({ pathname: '/chat/[chatId]', params: { chatId: chat.id } });
```

`chat/index.tsx`는 `mode` param과 React Query 캐시를 보고 내부에서 분기한다.

- `mode === 'new'` → 무조건 `NewChatScreen`
- mode 없음 → 캐시에서 최근 채팅 확인 → 있으면 `/chat/[chatId]`로 `replace`, 없으면 `NewChatScreen`

`chatId` 직접 진입은 `chat/[chatId].tsx`에서 처리한다.
...
```tsx
// app/(main)/chat/index.tsx — params + 캐시 분기가 필요한 경우
...
// app/(main)/chat/[chatId].tsx — params만 내려주는 경우
